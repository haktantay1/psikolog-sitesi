...
document.querySelectorAll('.pub-scroll-wrap').forEach(function(ps){
let down=false, startX, sl, moved=0, lastX=0;
ps.addEventListener('mousedown', e=>{down=true; moved=0; startX=e.pageX; lastX=e.pageX; sl=ps.scrollLeft;});
ps.addEventListener('mouseleave', ()=>down=false);
ps.addEventListener('mouseup', ()=>down=false);
ps.addEventListener('mousemove', e=>{
if(!down) return;
e.preventDefault();
moved+=Math.abs(e.pageX-lastX);
lastX=e.pageX;
ps.scrollLeft = sl-(e.pageX-startX);
});
ps.addEventListener('click', e=>{ if(moved>8){ e.preventDefault(); e.stopPropagation(); } }, true);
ps.addEventListener('wheel', e=>{ if(Math.abs(e.deltaY)>Math.abs(e.deltaX)){ ps.scrollLeft+=e.deltaY; e.preventDefault(); } }, {passive:false});
});
...
data.append('entry.653947926', subject); 
...
gl.uniform3f(uColor, 0.85, 0.72, 0.48); 
...
const rings = [
{ axis:'x', angle: Math.PI/2.6, color: [0.80, 0.66, 0.34] }, 
{ axis:'y', angle: Math.PI/2.6, color: [0.45, 0.55, 0.72] }, 
{ axis:'z', angle: Math.PI/2.6, color: [0.61, 0.40, 0.50] } 
];
...
})();(function(){var cards=document.querySelectorAll('.book-card[data-title]');if(!cards.length)return;var CACHE_KEY='haktan-book-covers-v3',cache={};try{cache=JSON.parse(localStorage.getItem(CACHE_KEY)||'{}')}catch(e){}try{localStorage.removeItem('haktan-book-covers-v1');localStorage.removeItem('haktan-book-covers-v2')}catch(e){}function saveCache(){try{localStorage.setItem(CACHE_KEY,JSON.stringify(cache))}catch(e){}}function cleanTitle(t){return t.replace(/\s+[Tt]\.?\s*\d+\s*$/,'').replace(/\s+\d+\s*$/,'').trim()}function pickOL(docs){if(!docs||!docs.length)return null;for(var i=0;i<docs.length;i++){if(docs[i].cover_i)return'https://covers.openlibrary.org/b/id/'+docs[i].cover_i+'-L.jpg'}return null}function pickGB(items){if(!items||!items.length)return null;for(var i=0;i<items.length;i++){var im=items[i].volumeInfo&&items[i].volumeInfo.imageLinks;if(im){var s=im.thumbnail||im.smallThumbnail;if(s)return s.replace(/^http:/,'https:').replace(/&edge=curl/,'').replace(/&zoom=\d+/,'&zoom=2')}}return null}function tryOL(q){var u='https://openlibrary.org/search.json?q='+encodeURIComponent(q)+'&limit=5';return fetch(u).then(function(r){return r.json()}).then(function(d){return pickOL(d.docs)}).catch(function(){return null})}function tryGB(q){var u='https://www.googleapis.com/books/v1/volumes?q='+encodeURIComponent(q)+'&maxResults=5';return fetch(u).then(function(r){return r.json()}).then(function(d){return pickGB(d.items)}).catch(function(){return null})}function lookupCover(t,a,manual){if(manual)return Promise.resolve(manual);var k=t+'|'+a;if(cache.hasOwnProperty(k))return Promise.resolve(cache[k]);var ct=cleanTitle(t);var q1=ct+' '+a;var q2=t+' '+a;return tryOL(q1).then(function(u){if(u)return u;return tryGB(q1)}).then(function(u){if(u)return u;if(q2!==q1)return tryOL(q2).then(function(u2){if(u2)return u2;return tryGB(q2)});return null}).then(function(u){cache[k]=u||null;saveCache();return u})}function loadCover(card){var t=card.dataset.title,a=card.dataset.author,manual=card.dataset.cover,cv=card.querySelector('.book-cover');if(!cv)return;cv.classList.add('loading');lookupCover(t,a,manual).then(function(u){cv.classList.remove('loading');if(!u){cv.classList.add('no-cover');return}var img=new Image();img.alt=t+' – '+a;img.decoding='async';img.onload=function(){cv.appendChild(img);cv.classList.add('has-cover')};img.onerror=function(){cv.classList.add('no-cover')};img.src=u})}if('IntersectionObserver' in window){var io=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting){loadCover(e.target);io.unobserve(e.target)}})},{rootMargin:'300px 0px'});cards.forEach(function(c){io.observe(c)})}else{cards.forEach(loadCover)}})();

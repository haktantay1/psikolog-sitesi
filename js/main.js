...
})();(function(){var cards=document.querySelectorAll('.book-card[data-title]');if(!cards.length)return;var CACHE_KEY='haktan-book-covers-v2',cache={};try{cache=JSON.parse(localStorage.getItem(CACHE_KEY)||'{}')}catch(e){}function saveCache(){try{localStorage.setItem(CACHE_KEY,JSON.stringify(cache))}catch(e){}}function tryOpenLibrary(t,a){var u='https://openlibrary.org/search.json?title='+encodeURIComponent(t)+'&author='+encodeURIComponent(a)+'&limit=1';return fetch(u).then(function(r){return r.json()}).then(function(d){if(d.docs&&d.docs[0]&&d.docs[0].cover_i){return'https://covers.openlibrary.org/b/id/'+d.docs[0].cover_i+'-L.jpg'}return null}).catch(function(){return null})}function tryGoogleBooks(t,a){var q='intitle:'+encodeURIComponent(t)+'+inauthor:'+encodeURIComponent(a);var u='https://www.googleapis.com/books/v1/volumes?q='+q+'&maxResults=1';return fetch(u).then(function(r){return r.json()}).then(function(d){if(d.items&&d.items[0]&&d.items[0].volumeInfo&&d.items[0].volumeInfo.imageLinks){var im=d.items[0].volumeInfo.imageLinks;var s=im.thumbnail||im.smallThumbnail;if(s){return s.replace(/^http:/,'https:').replace(/&edge=curl/,'').replace(/&zoom=\d+/,'&zoom=2')}}return null}).catch(function(){return null})}function lookupCover(t,a,manual){if(manual)return Promise.resolve(manual);var k=t+'|'+a;if(cache.hasOwnProperty(k))return Promise.resolve(cache[k]);return tryOpenLibrary(t,a).then(function(u){if(u)return u;return tryGoogleBooks(t,a)}).then(function(u){cache[k]=u||null;saveCache();return u})}function loadCover(card){var t=card.dataset.title,a=card.dataset.author,manual=card.dataset.cover,cv=card.querySelector('.book-cover');if(!cv)return;cv.classList.add('loading');lookupCover(t,a,manual).then(function(u){cv.classList.remove('loading');if(!u){cv.classList.add('no-cover');return}var img=new Image();img.alt=t+' – '+a;img.decoding='async';img.onload=function(){cv.appendChild(img);cv.classList.add('has-cover')};img.onerror=function(){cv.classList.add('no-cover')};img.src=u})}if('IntersectionObserver' in window){var io=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting){loadCover(e.target);io.unobserve(e.target)}})},{rootMargin:'300px 0px'});cards.forEach(function(c){io.observe(c)})}else{cards.forEach(loadCover)}})();

...
})();(function(){var cards=document.querySelectorAll('.book-card[data-title]');if(!cards.length)return;var CACHE_KEY='haktan-book-covers-v1',cache={};try{cache=JSON.parse(localStorage.getItem(CACHE_KEY)||'{}')}catch(e){}function saveCache(){try{localStorage.setItem(CACHE_KEY,JSON.stringify(cache))}catch(e){}}function lookupCover(t,a){var k=t+'|'+a;if(cache.hasOwnProperty(k))return Promise.resolve(cache[k]);var u='https://openlibrary.org/search.json?title='+encodeURIComponent(t)+'&author='+encodeURIComponent(a)+'&limit=1';return fetch(u).then(function(r){return r.json()}).then(function(d){var c=(d.docs&&d.docs[0]&&d.docs[0].cover_i)?d.docs[0].cover_i:null;cache[k]=c;saveCache();return c}).catch(function(){return null})}function loadCover(card){var t=card.dataset.title,a=card.dataset.author,cv=card.querySelector('.book-cover');if(!cv)return;cv.classList.add('loading');lookupCover(t,a).then(function(c){cv.classList.remove('loading');if(!c){cv.classList.add('no-cover');return}var img=new Image();img.alt=t+' – '+a;img.decoding='async';img.onload=function(){cv.appendChild(img);cv.classList.add('has-cover')};img.onerror=function(){cv.classList.add('no-cover')};img.src='https://covers.openlibrary.org/b/id/'+c+'-M.jpg'})}if('IntersectionObserver' in window){var io=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting){loadCover(e.target);io.unobserve(e.target)}})},{rootMargin:'300px 0px'});cards.forEach(function(c){io.observe(c)})}else{cards.forEach(loadCover)}})();
